...
export async function getHashtagFeed (name) {
    return connectionDB.query('SELECT posts.* FROM posts JOIN posts_hashtags AS p ON p.post_id = posts.id JOIN hashtags AS h ON h.id = p.hashtag_id WHERE h.name = $1;', [name]);
}